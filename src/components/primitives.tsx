...
export function Logo({ size = 48, showName = true }: { size?: number; showName?: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      <div className="grid shrink-0 place-items-center overflow-hidden" style={{ width: size, height: size }}>
        <img src={logoUrl} alt="Caytori" className="scale-[1.6] object-contain" style={{ width: size, height: size }} />
      </div>
      {showName && <span className="font-display text-xl font-700 tracking-tight">Caytori</span>}
    </div>
  );
}
          ...
          >
            <div className="rounded-[var(--radius)] neu p-7">
              <div className="flex items-start justify-between gap-4">
                ...
                <button onClick={onClose} aria-label="Close" className="neu-sm neu-press grid h-9 w-9 shrink-0 place-items-center rounded-full text-muted-foreground">
                  <X size={16} />
                </button>
              </div>
...
export function Button({
  children, onClick, variant = "primary", className = "", type = "button", size = "md",
}: {
  children: ReactNode;
  onClick?: () => void;
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      style={bg}
      className={`inline-flex items-center justify-center gap-2 rounded-full font-medium cursor-pointer transition-all duration-300 ${pad} ${styles[variant]} ${variant === "primary" || variant === "danger" ? "shadow-lg hover:brightness-110" : ""} ${className}`}
    >
      {children}
    </motion.button>
  );
...
export function StatusBadge({ status }: { status: Status }) {
  ...
  return (
    <span
      ...
      style={{ background: `color-mix(in srgb, ${m.token} 15%, transparent)`, color: m.token }}
    >
      <span className="h-1.5 w-1.5 rounded-full" style={{ background: m.token }} />
      {m.label}
    </span>
  );
}
...
export function Card({ children, className = "", inset = false }: { children: ReactNode; className?: string; inset?: boolean }) {
  return (
    <div className={`rounded-[var(--radius)] ${inset ? "neu-inset" : "neu"} ${className}`}>
      {children}
    </div>
  );
}
